...
%UPDATES
%
%   Rquared polynomials (22 June 2020) - polyfitn (John D'Errico)
%   Lines 71 and 72 (March 2021)
%   Line 71. DEMUSE creates epochs that are 23.9375 long, not 24. (March 21
...
% create epochs (end based on the smallest number of frmes of force or individuals spikes)
epochs = 1: 23.9375*fs:length(IPTs);
maxFrames = min([length(IPTs) length(ForceChannel)]);
idx = epochs>maxFrames;
if sum(idx) == 1
    epochs(epochs>maxFrames) = maxFrames; 
else
    epochs(end+1) = maxFrames;
end
% [filename length(IPTs) length(ForceChannel) epochs(end)]
...
for EP = 1:length(epochs)-1   % loop through the epochs
    ...
    for  MU = 1: length (MUPulses)               % loop through the motor units
        ...
        % low pass filter force data 
        fc = 6;                             % cut-off frequency 
        Wn =  fc/(fs/2);                    % Normalized cutoff frequency 
        n = 4;                              % filter order 
        [B,A] = butter(n/2,Wn,'low');       % designs a lowpass filter.
        ForceData = filter(B,A,ForceData);  % apply the butterworth filter 
        ...
        if isempty (Spikes)
            ...
            Rsq(MU) = NaN;
            warning on 
            warning ('No spikes found for Epoch %.f Montor Unit number %.f',EP,MU) 
            continue
        else
            x = Spikes;
            y = Vector (Spikes);
...
%         title (sprintf('Channel %d - 1.7Hz to 64Hz', MU))  
    ...
    if ~isempty (filename{1})
        ...
        save (filename{1}, 'PolynomialData' , 'PolynomialData_after500ms','Rsq');                                              % save data with the name given
